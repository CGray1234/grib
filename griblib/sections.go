...
func readMessage(gribFile io.Reader, section0 Section0) (*Message, error) {
	...
	for {
		...
		if sectionHead.ContentLength() > 0 {
			...
			case 7:
				message.Section7, err = ReadSection7(byteReader, sectionHead.ContentLength(), message.Section5, message.Section3)
			case 8:
				// end-section, return
				return &message, nil
			default:
...
func ReadSection5(f io.Reader, length int) (section Section5, err error) {
	...
	if section.DataTemplateNumber != 0 && section.DataTemplateNumber != 2 && section.DataTemplateNumber != 3 && section.DataTemplateNumber != 41 {
...
func ReadSection7(f io.Reader, length int, section5 Section5, section3 Section3) (section Section7, sectionError error) {
	defer func() {
	...
	if length != 0 {
		...
		case Data41:
			section.Data, sectionError = ParseData41(f, length, &x)
		default:
